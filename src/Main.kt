...
fun main(args: Array<String>) {
    ...
    for (i in 0 until nGames) {
        ...
        //averageTotalWin[i] += playerWallet
    ...
    //funProb.forEach { println((it/nGames)*10) }
    //var sum = 0.0
    //funProb.forEach { sum += (it/nGames)*10; println(sum) }
    //averageTotalWin.forEach { println(it/1000) }

    println(playerWallet)
    println(casinoWallet)

    println(wins.toDouble() / nGames)
    println(loses.toDouble() / nGames)
    println(draws.toDouble() / nGames)
}
...
open class Hand {
    data class HandType(var cards: MutableList<Card> = mutableListOf(), var comb: Combinations.Combination = Combinations.Combination.HighCard, var score: Double =  0.0)
    ...
    fun findCombinationsAndDistribute(): List<Combinations.Combination> {
        ...
        val bestRemainingComb = if(remainingCombs.contains(Combinations.Combination.OnePair)) Combinations.Combination.OnePair else Combinations.Combination.HighCard
...
class Combinations {
    ...
    fun checkHandComb(hand: List<Card>): List<Combination> {
        ...
        if(checkRoyalFlush(hand)) { handCombs.add(Combination.RoyalFlush) }
        else {
    ...
    private fun checkRoyalFlush(hand: List<Card>): Boolean {
        val listRoyal = listOf(Cards.CardName.Ace, Cards.CardName.King, Cards.CardName.Queen, Cards.CardName.Jack,Cards.CardName.N10)
        val arr = IntArray(5) { 0 }
        lateinit var color: Cards.Color
        var helpColor = false
        hand.forEach { arr[Cards.Color.values().indexOf(it.color)]++ }
        arr.forEachIndexed { ind, ar -> if (ar == 5 || (ar == 4 && joker)) { color = Cards.Color.values()[ind]; helpColor = true } }
    ...
    private fun checkFullHouse(hand: List<Card>): Boolean {
        ...
        var thereAce = false
        indOfTwo.forEach { if (Cards.CardName.values()[it] == Cards.CardName.Ace) { thereAce = true } }
